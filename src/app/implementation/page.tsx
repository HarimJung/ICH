...
export default function ImplementationPage() {
  ...
  return (
    <div className="container-content section-gap">
      ...
      <div className="mb-6">
        <h1 className="mb-2">Guideline Implementation Status</h1>
        <p className="text-textSecondary max-w-3xl">
          Track which ICH members have adopted each guideline into their
      ...
      <div className="card p-6 mb-8">
        <div className="grid grid-cols-3 gap-6 text-center">
          <div>
            ...
            <div className="mt-1 text-sm text-textSecondary">Guidelines</div>
          </div>
          <div>
            <div className="text-3xl font-bold text-primary">
              {members.length}
            </div>
            <div className="mt-1 text-sm text-textSecondary">Members</div>
          </div>
          <div>
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-background">
              <th className="sticky left-0 z-20 bg-background px-4 py-3 text-left font-medium text-textSecondary border-b border-r border-border min-w-[100px]">
                ID
              </th>
              <th className="sticky left-[100px] z-20 bg-background px-4 py-3 text-left font-medium text-textSecondary border-b border-r border-border min-w-[260px]">
                Title
              </th>
              <th className="px-4 py-3 text-center font-medium text-textSecondary border-b border-border min-w-[120px]">
                Category
              </th>
              <th className="px-4 py-3 text-center font-medium text-textSecondary border-b border-border min-w-[80px]">
                Step
              </th>
              {members
                .filter(
                ...
                .map((m) => (
                  <th
                    key={m.key}
                    className="px-4 py-3 text-center font-medium text-textSecondary border-b border-border min-w-[80px]"
                  >
                    <div>{m.label}</div>
          ...
          <tbody>
            {filtered.map((g) => {
              ...
              return (
                ...
                >
                  <td className="sticky left-0 z-10 bg-surface px-4 py-3 border-r border-border">
                    <Link
                  ...
                  <td className="sticky left-[100px] z-10 bg-surface px-4 py-3 border-r border-border">
                    <Link
                      href={`/guidelines/${encodeURIComponent(g.id)}`}
                      className="text-textPrimary font-medium hover:text-secondary line-clamp-1"
                    >
                      {g.title}
                    </Link>
                  </td>
                  ...
                  {visibleMembers.map((m) => (
                    <td key={m.key} className="px-4 py-3 text-center">
                      {g.implementationStatus[m.key] ? (
                        ...
                        <Minus className="inline h-5 w-5 text-textSecondary/30" />
                      )}
                    </td>
                  ))}
                </tr>
