...
export default function UpdatesPage() {
  ...
  const filtered = useMemo(() => {
    const sorted = [...typedUpdates].sort((a, b) =>
  ...
  const counts = useMemo(() => {
    ...
    for (const u of typedUpdates) {
      for (const f of filters) {
        if (f.value !== "all" && f.types.includes(u.type)) {
          c[f.value]++;
        }
      }
    }
  ...
  return (
    <div className="container-content section-gap">
      ...
      <nav className="mb-6 text-sm text-textSecondary">
        ...
        <span className="text-textPrimary font-medium">What&apos;s New</span>
      </nav>

      <div className="mb-8">
        <h1 className="mb-2">What&apos;s New</h1>
        <p className="text-textSecondary max-w-3xl">
          Stay informed about the latest developments across ICH guidelines,
          consultations, training and events.
        </p>
      </div>
        ...
        <div className="flex gap-0 min-w-max">
          {filters.map((f) => (
            <button
              ...
              className={`px-4 py-3 text-sm font-medium whitespace-nowrap border-b-[3px] transition-colors ${activeFilter === f.value
            ...
            >
              {f.label}
              <span
                className={`ml-1.5 rounded-full px-2 py-0.5 text-xs font-semibold ${activeFilter === f.value
      ...
      <div className="max-w-3xl">
        {grouped.map((group) => (
          <div key={group.month} className="mb-8">
            {/* Month label */}
            <h3 className="text-sm font-semibold text-textSecondary uppercase tracking-wide mb-4">
              {group.month}
            </h3>
              ...
              {group.items.map((u) => {
                ...
                return (
                  <div key={u.id} className="relative mb-6 last:mb-0">
                    {/* Dot */}
                    <div
                      className={`absolute -left-8 top-2 h-3 w-3 rounded-full ${config.dotColor} ring-2 ring-white`}
                    />
                    ...
                    <div className="card p-5">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {u.fromStep != null && u.toStep != null && (
                          <span className="flex items-center gap-1 text-xs font-semibold text-primary">
                            Step {u.fromStep}{" "}
                            <ArrowRight className="h-3 w-3" /> Step {u.toStep}
                          </span>
                        )}
                      ...
                      <Link
                        href={
                          u.guidelineId
                            ? `/guidelines/${encodeURIComponent(u.guidelineId)}`
                            : "/updates"
                        }
